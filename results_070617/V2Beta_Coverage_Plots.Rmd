...
```{r}
#read in file
betaV2<-read_tsv(here("results_beta_042921", "Targetome_FullEvidence_210602_All.txt"))
View(betaV2)
#clean up
...
#sms
interactionsSMS<- drugInteractions %>% 
                        filter(Database=="HiTS Small Molecule Suite") %>%
                        group_by(Drug_Found, Target_UniProt) 
numInteractionsSMS<-nrow(summarize(interactionsSMS, count=n()) )
#bind together, do alphabetical ordering for each database
statsInts<-rbind(numInteractionsBindingDB, numInteractionsDrugBank, numInteractionsIUPHAR, numInteractionsTTD, numInteractionsSMS)
...
#level ordering
fullStatsLong$Database<-factor(fullStatsLong$Database, levels = c("BindingDB", "DrugBank", "HiTS Small Molecule Suite", "IUPHAR", "Therapeutic Target Database", "All"))
...
imatinib_under100_AllAssays<-ggplot(imatinib_interactions_100_dis, #use for full assay plot
                                    aes(x=Assay_Value, fill=Target_Name))+
                                    scale_fill_manual(values = tol27rainbow_alt)+
                                    facet_grid(Assay_Type~.) + #facet by assay type
                                    geom_histogram(binwidth = 1.0, colour="black") + 
                                    ylab("Count") +
                                    xlab("Binding Assay Value (nM), under 100nM")+
...
#####Crizotinib
crizotinib_interactions<-drugInteractions%>% filter(Drug_Found=="Crizotinib")
crizotinib_interactions$Assay_Value<-as.numeric(as.character(crizotinib_interactions$Assay_Value))
crizotinib_interactions_100<-crizotinib_interactions %>% filter(Assay_Relation=="=" & Assay_Value <=100)
#View(crizotinib_interactions_100) 
crizotinib_interactions_100_dis<-crizotinib_interactions_100 %>% 
  distinct(Drug_Found, Target_UniProt, Target_Name, Database, Assay_Type, Assay_Value, Assay_Relation, Reference)
#View(crizotinib_interactions_100_dis)

crizotinib_interactions_AllAssays<-ggplot(crizotinib_interactions_100_dis, 
                                      aes(x=Assay_Value, fill=Target_Name))+
                                      scale_fill_manual(values = tol27rainbow)+
                                      facet_grid(Assay_Type~.) + #facet by assay type 
                                      geom_histogram(binwidth = 1.0, colour="black") + 
                                      ...
                                      theme(legend.key.size = unit(0.4, "cm"), 
                                            ...
                                            legend.title = element_text(size = 4, face = "bold"),
                                            legend.text = element_text(size = 4))
crizotinib_interactions_AllAssays
...
#####Olaparib
olaparib_interactions<-drugInteractions%>% filter(Drug_Found=="Olaparib")
olaparib_interactions$Assay_Value<-as.numeric(as.character(olaparib_interactions$Assay_Value))
olaparib_interactions_100<-olaparib_interactions %>% filter(Assay_Relation=="=" & Assay_Value <=100)
olaparib_interactions_100_dis<-olaparib_interactions_100 %>% 
  distinct(Drug_Found, Target_UniProt, Target_Name, Database, Assay_Type, Assay_Value, Assay_Relation, Reference)

olaparib_interactions_AllAssays<-ggplot(olaparib_interactions_100_dis, 
                                      aes(x=Assay_Value, fill=Target_Name))+
                                      scale_fill_manual(values = tol27rainbow)+
                                      facet_grid(Assay_Type~.) + #facet by assay type 
                                      geom_histogram(binwidth = 1.0, colour="black") + 
                                      ...
                                      theme(legend.key.size = unit(0.4, "cm"), 
                                            ...
                                            legend.title = element_text(size = 4, face = "bold"),
                                            legend.text = element_text(size = 4))
olaparib_interactions_AllAssays

#niraparib
niraparib_interactions<-drugInteractions%>% filter(Drug_Found=="Niraparib")
niraparib_interactions$Assay_Value<-as.numeric(as.character(niraparib_interactions$Assay_Value))
niraparib_interactions_100<-niraparib_interactions %>% filter(Assay_Relation=="=" & Assay_Value <=100)
niraparib_interactions_100_dis<-niraparib_interactions_100 %>% 
  distinct(Drug_Found, Target_UniProt, Target_Name, Database, Assay_Type, Assay_Value, Assay_Relation, Reference)

niraparib_interactions_AllAssays<-ggplot(niraparib_interactions_100_dis, 
                                      aes(x=Assay_Value, fill=Target_Name))+
                                      scale_fill_manual(values = tol27rainbow)+
                                      facet_grid(Assay_Type~.) + #facet by assay type 
                                      geom_histogram(binwidth = 1.0, colour="black") + 
                                      ...
                                      theme(legend.key.size = unit(0.4, "cm"), 
                                            ...
                                            legend.title = element_text(size = 4, face = "bold"),
                                            legend.text = element_text(size = 4))
niraparib_interactions_AllAssays
